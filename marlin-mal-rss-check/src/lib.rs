...
pub struct InterData<'a, F: PrimeField, PC: PolynomialCommitment<F, DensePolynomial<F>>, D: Digest> {
    /// Intermediate data between the first and second phase
    prover_first_msg: ProverMsg<F>,
    prover_first_oracles: ProverFirstOracles<F>,
    prover_state: ProverState<'a, F>,
    first_comms: Vec<LabeledCommitment<PC::Commitment>>,
    first_comm_rands: Vec<PC::Randomness>,
    verifier_first_msg: VerifierFirstMsg<F>,
    verifier_state: VerifierState<F>,
    fs_rng: FiatShamirRng<D>,
    public_input: Vec<F>,
}
...
impl<F: PrimeField, PC: PolynomialCommitment<F, DensePolynomial<F>>, D: Digest> Marlin<F, PC, D> {
    ...
    pub fn prove<C: ConstraintSynthesizer<F>, R: RngCore>(
        ...
        let open_time = start_timer!(|| "Openning combinations");
    ...
    /// Create a zkSNARK asserting that the constraint system is satisfied.
    pub fn prove_first_phase<'a, C: ConstraintSynthesizer<F>, R: RngCore>(
        index_pk: &'a IndexProverKey<F, PC>,
        c: C,
        zk_rng: &mut R,
    ) -> Result<InterData<'a, F, PC, D>, Error<PC::Error>> {
        let prover_time_first_phase = start_timer!(|| "Marlin::Prover");
        // Add check that c is in the correct mode.
        ...
        end_timer!(prover_time_first_phase);
    ...
    pub fn prove_second_phase<C: ConstraintSynthesizer<F>, R: RngCore>(
        index_pk: &IndexProverKey<F, PC>,
        interdata: InterData<F, PC, D>,
        zk_rng: &mut R,
    ) -> Result<Proof<F, PC>, Error<PC::Error>> {
        // --------------------------------------------------------------------
        // Second round

        let prover_time_second_phase = start_timer!(|| "Marlin::Prover");

        let (mut prover_second_msg, prover_second_oracles, prover_state) =
            AHPForR1CS::prover_second_round(&interdata.verifier_first_msg, interdata.prover_state, zk_rng);
        ...
        prover_second_msg.publicize();
        second_comms.publicize();
        let mut fs_rng = interdata.fs_rng;
        fs_rng.absorb(&to_bytes![second_comms, prover_second_msg].unwrap());

        let (verifier_second_msg, verifier_state) =
            AHPForR1CS::verifier_second_round(interdata.verifier_state, &mut fs_rng);
        // --------------------------------------------------------------------
        ...
        let polynomials: Vec<_> = index_pk
            .index
            .iter()
            .chain(interdata.prover_first_oracles.iter())
            .chain(prover_second_oracles.iter())
            .chain(prover_third_oracles.iter())
            .collect();
        ...
        #[rustfmt::skip]
        let commitments = vec![
            interdata.first_comms.iter().map(|p| p.commitment().clone()).collect(),
            second_comms.iter().map(|p| p.commitment().clone()).collect(),
            third_comms.iter().map(|p| p.commitment().clone()).collect(),
        ];
        let labeled_comms: Vec<_> = index_pk
            ...
            .zip(&AHPForR1CS::<F>::INDEXER_POLYNOMIALS)
            .map(|(c, l)| LabeledCommitment::new(l.to_string(), c, None))
            .chain(interdata.first_comms.iter().cloned())
            .chain(second_comms.iter().cloned())
            .chain(third_comms.iter().cloned())
            .collect();
        ...
        let comm_rands: Vec<PC::Randomness> = index_pk
            ...
            .clone()
            .into_iter()
            .chain(interdata.first_comm_rands)
            .chain(second_comm_rands)
            .chain(third_comm_rands)
            .collect();
        ...
        // Gather prover messages together.
        let prover_messages = vec![interdata.prover_first_msg, prover_second_msg, prover_third_msg];

        let proof = Proof::new(commitments, evaluations, prover_messages, pc_proof);
        proof.print_size_info();
        end_timer!(prover_time_second_phase);
        Ok(proof)
    }
