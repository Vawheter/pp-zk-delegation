...
impl<F: PrimeField, PC: PolynomialCommitment<F, DensePolynomial<F>>, D: Digest> Marlin<F, PC, D> {
    ...
    pub fn prove<C: ConstraintSynthesizer<F>, R: RngCore>(
        ...
        c: C,
        zk_rng: &mut R,
    ) -> Result<(Proof<F, PC>, ConsistencyCheckProof<F, PC>), Error<PC::Error>> {
        ...
        let eval_proof = PC::open(
            ...
            &[first_comm_rands[0].clone()],
            Some(zk_rng),
        ).map_err(Error::from_pc_err)?;
        let w_at_s = prover_first_oracles.w.clone().evaluate(&s);
        let consistency_check_proof = ConsistencyCheckProof {
            w_at_s,
            eval_proof,
        };
        // --------------------------------------------------------------------
        ...
        Ok((proof, consistency_check_proof))
    }
    ...
    /// Verify a consistency_check_proof
    pub fn consistency_check<C: ConstraintSynthesizer<F>, R: RngCore>(
        index_vk: &IndexVerifierKey<F, PC>,
        public_input: &[F],
        c_with_plain_witness: C,
        consistency_check_proof: &ConsistencyCheckProof<F, PC>,
        proof: &Proof<F, PC>,
        rng: &mut R,
    ) -> Result<bool, Error<PC::Error>> {
        let verifier_time = start_timer!(|| "Marlin::Verify_ConsistencyCheckProof");
        ...
        c_with_plain_witness.generate_constraints(pcs.clone()).unwrap();
