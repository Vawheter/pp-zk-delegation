...
impl<F: Field> ShareConversion for RSS3FieldShare<F> {
    type Target = AdditiveFieldShare<F>;

    fn share_conversion(self) -> Self::Target {
        // let pre_id = (dealer_id + 2) % 3;
        // let next_id = (dealer_id + 1) % 3;
        match Net::party_id() {
            0 => AdditiveFieldShare { val: self.val0 + self.val1},
            1 => AdditiveFieldShare { val: self.val1 },
