...
function usage {
  echo "Usage: $0 {groth16,marlin,plonk,marlin_mal} {hbc,spdz,gsz,rss3,local,ark-local} N_SQUARINGS N_PARTIES" >&2
  exit 1
}
...
case $proof in
    groth16|marlin|plonk|marlin_mal)
        ;;
    *)
        usage
esac
...
case $infra in
    hbc|spdz|gsz|rss3)
        ...
        do
          ...
          if [ $i -eq 0 ]
          then
            RUST_LOG=debug $BIN -p $proof -c squaring --computation-size $size mpc --hosts $NETWORK_CONFIG --party $i --alg $infra | rg "End: *$LABEL" | rg -o '[0-9][0-9.]*.s' &
            pid=$!
          else
