...
impl<G: AffineCurve, D: Digest, P: UVPolynomial<G::ScalarField>> InnerProductArgPC<G, D, P> {
    ...
    fn succinct_check<'a>(
        ...
        for (labeled_commitment, value) in labeled_commitments.zip(values) {
            ...
            combined_commitment_proj += &labeled_commitment.commitment().comm.scalar_mul(cur_challenge);
            ...
            if let Some(degree_bound) = degree_bound {
                ...
                combined_commitment_proj += &commitment.shifted_comm.unwrap().scalar_mul(cur_challenge);
            }
        ...
        if proof.hiding_comm.is_some() {
            ...
            combined_commitment_proj += &(hiding_comm.scalar_mul(hiding_challenge) - &vk.s.scalar_mul(rand));
        ...
        let mut round_commitment_proj = combined_commitment_proj + &h_prime.scalar_mul(&combined_v);
        ...
        for (l, r) in l_iter.zip(r_iter) {
            ...
            round_commitment_proj +=
                &(l.scalar_mul(round_challenge.inverse().unwrap()) + &r.scalar_mul(round_challenge));
        }
    ...
    P: UVPolynomial<G::ScalarField, Point = G::ScalarField>,
{
        ...
        P: 'a,
    {
        ...
        if has_hiding {
            ...
            combined_commitment_proj +=
                &(hiding_commitment.unwrap().scalar_mul(hiding_challenge) - &ck.s.scalar_mul(combined_rand));
